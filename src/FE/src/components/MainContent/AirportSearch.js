        ...
        <div className="list-airports" ref={listRef}>
          {Object.keys(filteredAirports).length > 0 ? (
            ...
            <div key={country} style={{ marginBottom: "10px" }}>
                <div style={{ marginTop: "20px", marginBottom: "20px", cursor: "pointer"}} className="row" onClick={() => toggleCountryVisibility(country)}>
                    <div className="country-cell">{country} ({filteredAirports[country].length})</div>
                    <span className={countryVisibilityState[country] ? "rotated" : ""}>
                        <svg width="12" height="6" viewBox="0 0 12 6" fill="none"><path opacity="0.9" fill-rule="evenodd" clip-rule="evenodd" d="M0 0L6 6L12 0L0 0Z" fill="#333333"></path></svg>
                    </span>
                </div>
